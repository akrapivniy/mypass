...
int flash_write (char *buffer, uint32_t size)
{
	flash_program_data(FLASH_OPERATION_ADDRESS, buffer, size);
	return 0;
}

int flash_read (char *buffer, uint32_t size)
{
	flash_read_data (FLASH_OPERATION_ADDRESS, buffer, size);
	return 0;
}
